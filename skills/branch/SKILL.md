...
- **type**: Infer from context — `feat/`, `fix/`, `chore/`, `docs/`, `refactor/`, `perf/`, `test/`, `ci/`
  - If an issue has labels like `bug` → `fix/`, `enhancement` → `feat/`, `documentation` → `docs/`
...
- Issue #45 labeled `bug`, titled "Parser crashes on empty input" → `fix/45-parser-crashes-on-empty-input`
